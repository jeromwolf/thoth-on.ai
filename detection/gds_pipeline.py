...
# 주의: Broker/Agent 허브 엣지(BROKERED/SOLD_POLICY)는 **정상 설계사·브로커도
# 무관한 다수 고객을 연결**해 Louvain 이 정상 고객을 거대 커뮤니티로 병합한다
# (정상 커뮤니티 신호 폭증·정밀도 파괴, 실측 정상비율 0.89). 따라서 조직형 사기
# 허브는 GDS 커뮤니티가 아니라 룰 허브 탐지(run_broker_hubs/run_agent_hubs,
# 정밀 ~1.0)로 처리하고, 커뮤니티 프로젝션에는 포함하지 않는다.
...
# WP-KR: 조직형 사기 허브(Broker/Agent)도 포함해 PageRank/Degree 로 허브 중심성을
# 측정한다(중심성은 커뮤니티 병합과 달리 정상 비율을 부풀리지 않음). 신규 노드/엣지가
# 없으면 _filter_projection 이 자동 제외한다(graceful).
_CENTRALITY_NODES = [
    "Customer", "Claim", "Account", "Hospital", "RepairShop",
    "Broker", "Agent", "Policy",
]
_CENTRALITY_RELS = {
    "FILED": {"orientation": "UNDIRECTED"},
    ...
    "BROKERED": {"orientation": "UNDIRECTED"},
    "SOLD_POLICY": {"orientation": "UNDIRECTED"},
}
...
def _existing_labels() -> set[str]:
    """그래프에 실제 존재하는 노드 레이블 집합."""
    try:
        rows = db.run("CALL db.labels() YIELD label RETURN collect(label) AS labels")
        return set(rows[0]["labels"]) if rows else set()
    except Exception:
        return set()


def _existing_rel_types() -> set[str]:
    """그래프에 실제 존재하는 관계 타입 집합."""
    try:
        rows = db.run(
            "CALL db.relationshipTypes() YIELD relationshipType "
            "RETURN collect(relationshipType) AS types"
        )
        return set(rows[0]["types"]) if rows else set()
    except Exception:
        return set()


def _filter_projection(
    nodes: list[str], rels: dict[str, Any]
) -> tuple[list[str], dict[str, Any]]:
    """존재하는 노드 레이블/관계 타입만 남긴다(신규 노드 미적재 데이터 graceful)."""
    have_labels = _existing_labels()
    have_rels = _existing_rel_types()
    f_nodes = [n for n in nodes if n in have_labels] or nodes
    f_rels = {k: v for k, v in rels.items() if k in have_rels} or rels
    return f_nodes, f_rels


def project_community_graph() -> dict[str, Any]:
    """군집 탐지용 프로젝션을 (재)생성한다. 노드/관계 수 반환."""
    drop_graph(COMMUNITY_GRAPH)
    nodes, rels = _filter_projection(_COMMUNITY_NODES, _COMMUNITY_RELS)
    rows = db.run(
...
def project_centrality_graph() -> dict[str, Any]:
    ...
    nodes, rels = _filter_projection(_CENTRALITY_NODES, _CENTRALITY_RELS)
    rows = db.run(
        "CALL gds.graph.project($name, $nodes, $rels) "
        "YIELD nodeCount, relationshipCount "
        "RETURN nodeCount, relationshipCount",
