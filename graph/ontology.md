...
| 온톨로지 버전 | **v0.2** |
| 범위 | 자동차보험 사기 PoC (crash-for-cash 중심) |
| 최종 수정 | 2026-06-16 |
...
`graph/01_schema.cypher` L8–16 의 노드 온톨로지와 정확히 일치한다.
...
`graph/01_schema.cypher` L19–29 의 엣지 온톨로지와 정확히 일치한다. 카디널리티는 PoC 도메인 규칙 기준 표기(`1`=정확히 1, `0..1`=선택, `*`=다수).
