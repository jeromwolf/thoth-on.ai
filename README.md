...
```
> **피드백 루프(서빙까지 폐쇄):** 조사관 판정(FRAUD/NORMAL)을 운영 라벨로 환원해 모델을 재학습하고(`POST /detection/retrain`, 콘솔 재학습 패널), `persist=true` 시 모델을 영속화(`detection/model_store.py`, joblib)해 **라이브 스코어링에 실제 반영**한다(`score_customers(use_ml=True)` → `ML_FRAUD_PROB` corroborating 신호). 활성 모델은 `GET /detection/model`로 확인하고, `POST /detection/rescore`(콘솔 "큐에 반영" 버튼)로 **케이스 큐 점수를 재산출**해 조사관 화면에 반영한다(`CaseStore.update_score`). **라벨(y)만 조작·피처(X) 불변으로 누수 차단**. 정직성: ① baseline(ground truth)과 feedback(판정 라벨)은 서로 다른 라벨 집합이라 delta는 참고치, ② `use_ml` 스코어링은 *서빙* 경로로 본질상 in-sample이며 일반화 성능은 `detection.ml_model`의 out-of-fold가 별도 측정한다.
